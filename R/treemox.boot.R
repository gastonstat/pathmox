#'@title Bootstrapping validation on path coefficients of terminal nodes from a
...
#'following results:
#'@return \item{PC}{Matrix of original path coefficients for the root node and the
#'terminal nodes.}
#'@return \item{PMB}{Matrix of bootstrap path coefficients (mean value) for the
#'root node and the terminal nodes.}
#'@return \item{PSB}{Matrix of bootstrap standard errors of path coefficients for
#'the root node and the terminal nodes.}
#'@return \item{PP05}{Matrix of 0.05 bootstrap percentile of path coefficients for
#'the root node and the terminal nodes.}
#'@return \item{PP95}{Matrix of 0.95 bootstrap percentile of path coefficients for
#'the root node and the terminal nodes.}
...
function(pls, treemox, X=NULL, br=100)
{
  ...
  if (nrow(pls$latents)!=treemox$MOX$Size[1]) 
    stop("\nOops, 'pls' and 'pathmox' are incompatible. Different number of observations")
  if (!is.null(X)) # if X available
  {
    if (is.null(pls$data))
    {
      ...
      if (nrow(X)!=pathmox$MOX$Size[1])
        stop("\nOops, 'X' and 'pathmox' are incompatible. Different number of observations")
    }
  } else { # if no X
    if (is.null(pls$data))
  ...
  if (mode(br)!="numeric" || length(br)!=1 || (br%%1)!=0 ||
    br<50 || br>500) {
    warning("\n'br' must be an integer between 50 and 500. Default 'br=100' is used.")   
    br <- 100
  } 
  ...
  {        
    ...
    DTN <- DT[elems.tn,]
    pls.tn <- get_pls_basic(DTN, IDM, blocks, modes, scheme, scaled, tol, iter)
